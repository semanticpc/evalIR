...
checkURL <- function(url){
  print(url)
  return(try(url.exists(url)))
}
pool_documents <- function(depth){
  trec09 <- c('../../runs/trec/trec2009')
  trec10 <- c('../../runs/trec/trec2010')
  trec11 <- c('../../runs/trec/trec2011')
  ...
  for(q in sort(runs$getQueries(), decreasing=T)){
    ...
    if ( nrow(df_urls) > 0 ){
      ...
      status <- sapply(doc_urls$URL, function(x) checkURL(x))
      
      doc_urls_status <- cbind(doc_urls, status)
      #doc_urls_status <- doc_urls
      rownames(doc_urls_status) <- c()
      res <- rbind(doc_urls_status, res)
    } else{
    ...
    save(res, file=paste('RData/', q, '.RData', sep=''))
...
getURLs <- function(docids){
  con <- dbConnect(MySQL(),
                   user="root", password="",
                   dbname="clueweb09", host="127.0.0.1", port=3666)
  
  rs <- dbSendQuery(con, 
                    paste("SELECT * FROM clueweb_urls WHERE docID in (", 
