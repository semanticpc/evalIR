#include "AdhocQrels.hpp"

using namespace Rcpp;
using namespace std;

AdhocQrels::AdhocQrels(string qrelsPath) {
...
vector<int> AdhocQrels::getQueries() {
    vector<int> queries;
...
SEXP AdhocQrels::getGrades(int queryID) {
...
void AdhocQrels::setGrades(int queryID, vector<string> documents, vector<double> grades) {
    map<int, Qrels>::iterator it = qrels_list.find(queryID);
    if (it != qrels_list.end()) {
...
SEXP AdhocQrels::getNonRelDocuments(int queryID) {
    map<int, Qrels>::iterator it = qrels_list.find(queryID);
    if (it != qrels_list.end())
...
SEXP AdhocQrels::judgePair(int queryID, CharacterMatrix pairs, string ties) {
    map<int, Qrels>::iterator it = qrels_list.find(queryID);
    srand(time(NULL));
...
SEXP AdhocQrels::judgeQuery(int queryID, vector<string> run) {
    map<int, Qrels>::iterator it = qrels_list.find(queryID);
