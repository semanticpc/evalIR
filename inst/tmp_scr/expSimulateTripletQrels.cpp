...
SimulateTripletQrels::SimulateTripletQrels(string _qrelsPath,
    ...
    set<string> resolveTiesMethods;
    resolveTiesMethods.insert("random");
    resolveTiesMethods.insert("alphabetical");
    resolveTiesMethods.insert("tie");

    set<string> simulationMethods;
    simulationMethods.insert("subtopic");
    simulationMethods.insert("subtopicLR23");

    if (opts.containsElementNamed("simulation"))
        simulationMethod = Rcpp::as<std::string>(opts["simulation"]);

    if (opts.containsElementNamed("ties"))
        resolveTiesMethod = Rcpp::as<std::string>(opts["ties"]);

    if (simulationMethods.find(simulationMethod) == simulationMethods.end())
        simulationMethod = "subtopic";
    if (resolveTiesMethods.find(resolveTiesMethod) == resolveTiesMethods.end())
...
string SimulateTripletQrels::judgeTriplet(int query,
    ...
    if (simulationMethod == "subtopic") {
        pref = sim_Subtopic(query, top_docid, left_docid, right_docid);
    } else if (simulationMethod == "subtopicLR23")
        pref = sim_SubtopicLR23(query, top_docid, left_docid, right_docid);

    if (pref == "tie" && resolveTiesMethod != "tie") {
        if (resolveTiesMethod == "random")
            return ties_random(left_docid, right_docid);
...
string SimulateTripletQrels::sim_SubtopicLR23(int query,
        vector<string> top_docid, string left_docid, string right_docid) {
    ...
    if (arma::sum(right) == 0 && arma::sum(left) == 0 && arma::sum(top) == 0)
        return "-3";
    else if (arma::sum(right) == 0 && arma::sum(left) == 0)
        return "-2";
...
string SimulateTripletQrels::sim_Subtopic(int query,
        vector<string> top_docid, string left_docid, string right_docid) {
    vec left = div_qrels[query].getGrade(left_docid);
    vec right = div_qrels[query].getGrade(right_docid);
    vec top = arma::zeros(div_qrels[query].st_prob.size());
...
string SimulateTripletQrels::ties_random(string left, string right) {
...
RCPP_MODULE(SimulateTripletQrels) {
    ...
    class_<SimulateTripletQrels > ("SimulateTripletQrels")
            ...
            .method("judgeTriplet", &SimulateTripletQrels::judgeTriplet)
