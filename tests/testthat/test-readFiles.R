...
test_that("Read Qrels: Ad-hoc", {
  qrelsFile <- system.file('test_data/qrels/toy.qrels', package='evalIR')
  qrels <- read.qrels(qrelsFile, type='adhoc')
  
  nonrelDocs301 <- c('DocA','DocB','DocF', 'DocG', 'DocH', 'DocI', 'DocN')
  relDocs301 <- c('DocC', 'DocD', 'DocE', 'DocJ', 'DocK', 'DocL', 'DocM')
...
test_that("TREC Read Run: getRunids and getQueries", {
  runFile1 <- system.file('test_data/runs/result.test', package='evalIR')
  runFile2 <- system.file('test_data/runs/result.trunc', package='evalIR')
  
  runs <- read.runs(c(runFile1, runFile2))
  # Testing getQueries and getRunids
  expect_equal(sort(runs$getQueries()), c(301, 302, 303))
  expect_equal(sort(runs$getRunids()), c('result.test', 'result.trunc'))
